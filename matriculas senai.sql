...
(SELECT 
	...
	CASE 
		WHEN SCURSO.CURPRESDIST = 'D' THEN 'À Distância' ELSE 'Presencial' END MAT_ACAO,
	LINHAACAO.DESCRICAO MAT_TIPOAMBIENTE,
	CASE 
...
LEFT JOIN GMUNICIPIO ON GMUNICIPIO.CODMUNICIPIO = GFILIAL.CODMUNICIPIO AND GMUNICIPIO.CODETDMUNICIPIO = GFILIAL.ESTADO
...
LEFT JOIN GCONSIST LINHAACAO(NOLOCK) ON STURMACOMPL.LINHACAO = LINHAACAO.CODINTERNO AND LINHAACAO.CODTABELA = 'LINHASACAO'
LEFT JOIN GCONSIST TIPOGRAT(NOLOCK) ON SHABILITACAOALUNOCOMPL.TIPOGRAT = TIPOGRAT.CODINTERNO AND TIPOGRAT.CODTABELA = 'TIPOGRAT'
LEFT JOIN GCONSIST INDICADOR(NOLOCK) ON SHABILITACAOALUNOCOMPL.PRONATEC = INDICADOR.CODINTERNO AND INDICADOR.CODTABELA = 'INDICADOR'
