	...
	CASE
		WHEN (CONVERT(DATE,SLOGPLETIVO.DTALTERACAO) > @DATAATE AND CONVERT(DATE,SHABILITACAOALUNOCOMPL.DATAFINAL) > @DATAATE)
		   ...
		   (SELECT SLG.CODSTATUS 
				...
				AND SLG.CODSTATUS != 1 
				AND SLG.DTALTERACAO = (SELECT MAX(SLP.DTALTERACAO) FROM SLOGPLETIVO SLP
									   	WHERE	SLP.CODCOLIGADA = SMATRICPL.CODCOLIGADA
...
WHERE  
    ...
    AND STURMA.CODTURMA = 'AIT-ELT-1-V1'
    AND STURMA.CODFILIAL = 6
	AND SMATRICPL.CODSTATUS != 1
    --AND SMATRICPL.RA  = '0041941'
    
) TAB
--WHERE TAB.MAT_SITUACAO IS NULL