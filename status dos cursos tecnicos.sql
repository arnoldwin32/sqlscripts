...
SET @DATADE = '2017-01-11'
SET @DATAATE = '2017-30-11'
...
(SELECT 
	...
	CASE
		WHEN SMODALIDADECURSO.CODMODALIDADECURSO NOT IN(15,31) 
			THEN
				CASE
					WHEN 
						(CONVERT(DATE,SLOGPLETIVO.DTALTERACAO) > @DATAATE AND CONVERT(DATE,SHABILITACAOALUNOCOMPL.DATAFINAL) > @DATAATE) 
					THEN 
					   (SELECT DISTINCT MAX(SLG.CODSTATUS)
					...
					THEN 
					   (SELECT DISTINCT MAX(SLG.CODSTATUS)
							...
							AND SLG.DTALTERACAO 		= (SELECT MAX(SLP.DTALTERACAO) 
															...
															AND SLP.RA 							= SMATRICPL.RA
															AND CONVERT(DATE,SLP.DTALTERACAO) 	<= @DATAATE))
					WHEN CONVERT(DATE,SHABILITACAOALUNOCOMPL.DATAFINAL) BETWEEN @DATADE AND @DATAATE THEN SMATRICPL.CODSTATUS
	...
	CASE WHEN PPESSOA.GRAUINSTRUCAO IN ('2','3','4') THEN '1'
	     ...
	     WHEN PPESSOA.GRAUINSTRUCAO	='1' THEN '0'
...
JOIN SHABILITACAOALUNO (NOLOCK) ON 	
...
JOIN SHABILITACAOALUNOCOMPL (NOLOCK) ON 
...
JOIN SALUNO (NOLOCK) ON 
	 SALUNO.CODCOLIGADA = SHABILITACAOALUNO.CODCOLIGADA AND 
	 SALUNO.RA = SHABILITACAOALUNO.RA

JOIN SALUNOCOMPL (NOLOCK) ON 
	 SALUNOCOMPL.CODCOLIGADA = SALUNO.CODCOLIGADA AND 
...
LEFT JOIN SCONTRATO (NOLOCK) ON 
	 ...
	 SCONTRATO.CODCONTRATO 				= (SELECT MAX(CODCONTRATO)FROM SCONTRATO SC(NOLOCK)
...
LEFT JOIN SRESPONSAVELCONTRATO (NOLOCK) ON 
		  SRESPONSAVELCONTRATO.CODCOLIGADA 	= SCONTRATO.CODCOLIGADA AND 
		  SRESPONSAVELCONTRATO.RA 			= SCONTRATO.RA AND 
		  SRESPONSAVELCONTRATO.CODCONTRATO 	= SCONTRATO.CODCONTRATO AND 
		  SRESPONSAVELCONTRATO.IDPERLET 	= SCONTRATO.IDPERLET

LEFT JOIN SLOGPLETIVO (NOLOCK) ON 
		  SLOGPLETIVO.CODCOLIGADA 			= SMATRICPL.CODCOLIGADA AND
		  SLOGPLETIVO.IDPERLET 				= SMATRICPL.IDPERLET AND 
		  SLOGPLETIVO.IDHABILITACAOFILIAL 	= SMATRICPL.IDHABILITACAOFILIAL AND
		  SLOGPLETIVO.CODTURMA 				= SMATRICPL.CODTURMA AND
		  SLOGPLETIVO.RA 					= SMATRICPL.RA AND
		  SLOGPLETIVO.DTALTERACAO 			= (SELECT MAX( ULTIMAALTERACAO.DTALTERACAO)FROM SLOGPLETIVO (NOLOCK) ULTIMAALTERACAO
...
LEFT JOIN GFILIAL (NOLOCK) ON 
		  GFILIAL.CODCOLIGADA	= SMATRICPL.CODCOLIGADA AND 
		  GFILIAL.CODFILIAL		= SMATRICPL.CODFILIAL
		  
LEFT JOIN FCFO (NOLOCK) ON 
		  FCFO.CODCOLIGADA	= SALUNO.CODCOLIGADA AND
		  FCFO.CODCFO		= SALUNO.CODCFO
		
LEFT JOIN FCFO EMPRESARESP (NOLOCK) ON 
		  EMPRESARESP.CODCOLIGADA 	= SRESPONSAVELCONTRATO.CODCOLCFO	AND 
		  EMPRESARESP.CODCFO 		= SRESPONSAVELCONTRATO.CODCFO
		  
LEFT JOIN GMUNICIPIO (NOLOCK) ON 
		  FCFO.CODMUNICIPIO = GMUNICIPIO.CODMUNICIPIO
		  
JOIN PPESSOA (NOLOCK) ON 
     PPESSOA.CODIGO = SALUNO.CODPESSOA

JOIN STURMA (NOLOCK) ON 
	 ...
	 STURMA.IDPERLET =  SMATRICPL.IDPERLET AND
	 STURMA.IDHABILITACAOFILIAL = SMATRICPL.IDHABILITACAOFILIAL

								 
JOIN STURMACOMPL (NOLOCK) ON 
...
WHERE  
	...
	AND STURMA.CODTURMA = 'EMP-INSTMED-04'
	AND SMATRICPL.CODFILIAL = 3
	AND STURMA.IDPERLET = (SELECT MAX(STM.IDPERLET) FROM SMATRICPL STM
					   WHERE  STM.CODCOLIGADA = STURMA.CODCOLIGADA
					   AND    STM.CODFILIAL = STURMA.CODFILIAL
					   AND    STM.IDHABILITACAOFILIAL = STURMA.IDHABILITACAOFILIAL
					   AND    STM.RECCREATEDON <= @DATAATE)
	) TAB 
